...
import static github.acodervic.mod.db.anima.core.AnimaCache.getGetterName;
import static github.acodervic.mod.db.anima.utils.Functions.ifThen;
        ...
        if (null != sqlParams.getUpdateColumns() && !sqlParams.getUpdateColumns().isEmpty()) {
            ...
            if (null != sqlParams.getModel()) {
                for (Field field : AnimaCache.computeModelFields(sqlParams.getModelClass())) {
                    try {
                        // Object value = AnimaUtils.invokeMethod(sqlParams.getModel(),
                        // getGetterName(field.getName()), AnimaUtils.EMPTY_ARG);
                        // 修复:当更新的时候有字段被置null无法修改至数据库
                        // if (null == value) {
                        // continue;
                        // }

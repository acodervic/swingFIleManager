...
public class IpUtil {
    ...
    public static void main(String[] args) {
        //System.out.println(parseIpRange("192.168.1.1", "192.168.1.55"));
        //System.out.println(parseIpRange("192.168.1.1/24"));
        //System.out.println(parseIpRange("192.168.1.1-4"));
        //System.out.println(parseIpRange("192.168.1.1-192.168.1.3,192.168.1.4-6,192.168.1.1/24"));
        System.out.println(parseIpRange("192.168.1.1,192.168.1.3"));
    ...
    /**
     * 获取ip范围的所有ip  调用:parseIpRange("192.168.1.1-24")
     * @param ipfrom
     * @return
     */
    public static List<String> parseIpRange(String ipfrom) {
        List<String> ret=new ArrayList<>();
        if (ipfrom.indexOf(",")>-1) {
            System.out.println("解析ip4异常其中不能包含 逗号  你可能需要提前分割!");
            String[] ipRanges = ipfrom.split(",");
            for (String ipRange : ipRanges) {
                if (ret.size()>9999) {
                    System.out.println("解析ip可能出现异常出现大量ip,停止解析");
                    break;
                }
                ret.addAll(parseIpRange(ipRange));
            }
            //对List进行去重
            ListUtil.duplicate(ret, (obj1,obj2)  ->{
                return obj1.equals(obj2);
            });
            return ret;
        }
        if (isIP(ipfrom)) {//就是一个普通的ip直接返回
            ret.add(ipfrom);
        }
        str str = new str(ipfrom);
        int showCount = str.showCount("\\.");//.的数量
        if (showCount==3&&ipfrom.indexOf("-")>-1) {//如果只有三个点,而且有一个 -  192.168.1.1-24
            //解析格式 192.168.1.1-24
            int lastIndexOf =str.lastIndexOf(".");
            str  baseIp3=str.sub(0, lastIndexOf+1);
            str subToEnd = str.subToEnd(lastIndexOf, false);
            AList<str> split = subToEnd.split("-");
            Integer startIp= split.get(0).to_I();
            Integer endIp= split.get(1).to_I();
            str fullStartIp=baseIp3.clone().insertStrToEnd(startIp.toString());
            str fullEndIp=baseIp3.clone().insertStrToEnd(endIp.toString());
            //计算范围
            ret.addAll(parseIpRange(fullStartIp.to_s(), fullEndIp.to_s()));
        }else if(showCount==3&&ipfrom.indexOf("/")>-1){//如果只有三个.,而且只有一个/  192.168.1.0/24
            int lastIndexOf =str.lastIndexOf(".");
            str  baseIp3=str.sub(0, lastIndexOf+1);
            str subToEnd = str.subToEnd(lastIndexOf, false);
            AList<str> split = subToEnd.split("/");
            Integer startIp= split.get(0).to_I();
            str mask= split.get(1);
            str fullStartIp=baseIp3.clone().insertStrToEnd(startIp.toString());
            ret.addAll(parseIpMaskRange(fullStartIp.to_s(), mask.to_s()));
        }else if(showCount==6&&ipfrom.indexOf("-")>-1){//如果有六个点 而且有一个-  可能的ip格式: 192.168.1.1-192.168.1.2
             AList<str> split = str.split("-");
             ret.addAll(parseIpRange(split.get(0).to_s(), split.get(1).to_s()));
        }

        return ret;
    }
    /***
     * 获取ip范围的所有ip  调用:parseIpRange("192.168.1.1", "192.168.1.55")
     * @param ipfrom
     * @param ipto
     * @return
     */
